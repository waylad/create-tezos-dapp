...
export class FA1_2 extends Helpers {
  ...
  approve(params: ApproveParams): void {
    ...
    Sp.verify(
      this.storage.balances.get(Sp.sender).approvals.get(params.spender, 0) ==
        0 || params.value == 0,
    ...
    this.storage.balances
      .get(Sp.sender)
      .approvals.set(params.spender, params.value);
  }
  ...
  burn(params: BurnParams): void {
    ...
    this.storage.balances.get(params.address).balance =
      (this.storage.balances.get(params.address).balance -
        params.value) as TNat;
    this.storage.totalSupply = (this.storage.totalSupply -
      params.value) as TNat;
  }
  ...
  transfer(params: TransferParams): void {
    ...
    Sp.verify(
      params.from === Sp.sender ||
        this.storage.balances.get(params.from).approvals.get(Sp.sender) >=
          params.value,
    ...
    Sp.verify(
      this.storage.balances.get(params.from).balance >= params.value,
      ErrorCodes.InsufficientBalance
    );

    this.storage.balances.get(params.from).balance = (this.storage.balances.get(
      params.from
    ).balance - params.value) as TNat;
    ...
    if (params.from !== Sp.sender) {
      const approval = (this.storage.balances
        .get(params.from)
        .approvals.get(Sp.sender) - params.value) as TNat;
  ...
  @EntryPoint
  getAllowance(
    request: TTuple<[{ owner: TAddress; spender: TAddress }, TContract<TNat>]>
  ): void {
    let allowance = 0;
    if (this.addressExists(request.fst().owner)) {
      allowance = this.storage.balances
        .get(request.fst().owner)
        .approvals.get(request.fst().spender, 0);
    }
...
Dev.test({ name: 'FA1_2' }, () => {
  ...
  Scenario.transfer(
    c1.mint({
      address: 'KT18amZmM5W7qDWVt2pH6uj7sCEd3kbzLrHT',
      value: 1000000,
    }),
    {
